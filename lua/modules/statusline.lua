...
function SlComponent.diagnosticStatus()
	...
	if errors > 0 then
		return hiPattern:format("Error", " E:" .. errors .. " ")
	end

	local warnings = #vim.diagnostic.get(0, { severity = diagnosticSeverity.WARN })
	if warnings > 0 then
		return hiPattern:format("Question", " W:" .. warnings .. " ")
	end
...
function SlComponent.fileName()
	return hiPattern:format("Directory", "%f%m")
end
...
function SlComponent.lspNames()
	...
	return hiPattern:format("Directory", lspNames .. " ")
end
